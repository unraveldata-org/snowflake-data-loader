...
AS
CALL REPLICATE_ACCOUNT_USAGE(DB_NAME => 'UNRAVEL_SHARE', SCHEMA_NAME => 'SCHEMA_4827_T', LOOK_BACK_DAYS => 2, TABLE_GROUP => 'HISTORY');

CREATE OR REPLACE TASK replicate_metadata_snapshots
    WAREHOUSE = UNRAVELDATA
    SCHEDULE = 'USING CRON 0 3,9,15,21 * * * UTC'
AS
CALL REPLICATE_ACCOUNT_USAGE(DB_NAME => 'UNRAVEL_SHARE', SCHEMA_NAME => 'SCHEMA_4827_T', LOOK_BACK_DAYS => 2, TABLE_GROUP => 'SNAPSHOTS');
