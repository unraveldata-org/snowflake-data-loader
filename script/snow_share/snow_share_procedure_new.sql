...
function insertToTable(destnTableName, sourceTableName, isDate, startDateCol, endDateCol, columns, primaryKeys)
{
    try {
        let insertQuery = "";
        const insertTable = `${dbName}.${schemaName}.${destnTableName}`;
...
try {
    const targetTable = `${dbName}.${schemaName}.IS_QUERY_HISTORY`;
...
function getColumns(tableName)
{
    var columns = "";
    var columnQuery =
        `SELECT LISTAGG(column_name, ', ') WITHIN GROUP (ORDER BY ordinal_position) as ALL_COLUMNS FROM
        ${DBNAME}.INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '${tableName}'
        AND TABLE_SCHEMA ='${SCHEMANAME}'`;
...
try {
    ...
    while (resultSet.next()) {
        ...
        var deleteRealtimeQueryByWh =
            `DELETE FROM ${DBNAME}.${SCHEMANAME}.IS_QUERY_HISTORY WHERE WAREHOUSE_NAME = '${whName}'`;
        ...
        var insertRealtimeQuery =
            `INSERT INTO ${DBNAME}.${SCHEMANAME}.IS_QUERY_HISTORY
            SELECT ${selectList}
...
var sourceDb = (SOURCE_DB && String(SOURCE_DB).trim().length > 0) ? String(SOURCE_DB).trim() : null;
// if SOURCE_SCHEMA is NULL/empty -> original flow
var sourceSchema = (SOURCE_SCHEMA && String(SOURCE_SCHEMA).trim().length > 0) ? String(SOURCE_SCHEMA).trim() : null;
...
queries[0] = `
    CREATE TRANSIENT TABLE IF NOT EXISTS ${dbName}.${schemaName}.QUERY_PROFILE (
...
queries[1] = `ALTER TABLE ${dbName}.${schemaName}.QUERY_PROFILE ADD COLUMN IF NOT EXISTS status_date DATE`;
...
if (sourceDb && sourceSchema) {
    ...
    const tgtTable = `${dbName}.${schemaName}.QUERY_PROFILE`;
    ...
    queries[2] = `
    CREATE OR REPLACE TEMPORARY TABLE ${dbName}.${schemaName}.QUERY_HISTORY_TEMP AS
    SELECT
      query_id,
    ...
    queries[3] = `
    SELECT COUNT(1)
    FROM ${dbName}.${schemaName}.QUERY_HISTORY_TEMP
    `;
    ...
    var actualQueryId = `
        SELECT tmp.query_id
        FROM ${dbName}.${schemaName}.query_history_temp tmp
        WHERE NOT EXISTS
        (SELECT query_id FROM ${dbName}.${schemaName}.QUERY_PROFILE qp WHERE qp.query_id = tmp.query_id);`;

    var selectExprs = buildSelectLists(columns,"AS_IS");

    var profileInsert = `INSERT INTO ${dbName}.${schemaName}.QUERY_PROFILE select ${selectExprs} from table(get_query_operator_stats(?))`;
...
try {
    if (sourceDb && sourceSchema) {
        ...
        const tgtWh = `${dbName}.${schemaName}.WAREHOUSES`;
        const tgtWp = `${dbName}.${schemaName}.WAREHOUSE_PARAMETERS`;
        ...
        // PK assumptions:
        // - WAREHOUSES: "name" is stable PK from SHOW WAREHOUSES output -> column exists in your target table as "name"
        // - WAREHOUSE_PARAMETERS: (WAREHOUSE, KEY) composite PK
        // If your WAREHOUSES table uses a different PK column than "name", replace t."name"/w."name" accordingly.
        exec(`
          INSERT INTO ${tgtWh}
          SELECT ${whSelect}
          FROM ${srcWh} w
    ...
    create_stmt.execute();
    var alterTable = `ALTER TABLE ${DBNAME}.${SCHEMANAME}.WAREHOUSES ADD COLUMN IF NOT EXISTS status_date DATE `;
    snowflake.createStatement({sqlText: alterTable}).execute();
    ...
    // 6. INSERT INTO
    var insert_sql_wh = `INSERT INTO "${DBNAME}"."${SCHEMANAME}".WAREHOUSES (${column_names.join(", ")}, status_date)
...
try {
    ...
    var createWP = 'CREATE TRANSIENT TABLE IF NOT EXISTS ' + DBNAME + '.' + SCHEMANAME + '.WAREHOUSE_PARAMETERS (WAREHOUSE VARCHAR(1000), KEY VARCHAR(1000), VALUE VARCHAR(1000), DEFAULT VARCHAR(1000),LEVEL VARCHAR(1000), DESCRIPTION VARCHAR(10000),TYPE VARCHAR(100), status_date DATE);';
    snowflake.createStatement({ sqlText: createWP }).execute();

    var alterTable2 = `ALTER TABLE ${DBNAME}.${SCHEMANAME}.WAREHOUSE_PARAMETERS ADD COLUMN IF NOT EXISTS status_date DATE `;
...
try {
    ...
    while (resultSet1.next()) {
        ...
        var wpInsert = `
          INSERT INTO "${DBNAME}"."${SCHEMANAME}".WAREHOUSE_PARAMETERS
...
try {
  ...
  insertToReplicationLog("completed", "replicate shared_db completed", task);
  return "success";

} catch (err) {
  try { logError(err, taskDetails); } catch (e) {}
