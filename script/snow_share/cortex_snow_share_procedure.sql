/**
    ...
    Tables covered by this script (date column used for incremental filtering):
        ...
        10. CORTEX_AGENT_USAGE_HISTORY                -> START_TIME
        11. CORTEX_AI_FUNCTIONS_USAGE_HISTORY         -> START_TIME
        12. CORTEX_SEARCH_BATCH_QUERY_USAGE_HISTORY   -> START_TIME
        13. CORTEX_CODE_SNOWSIGHT_USAGE_HISTORY       -> USAGE_TIME
        14. CORTEX_CODE_CLI_USAGE_HISTORY             -> USAGE_TIME
        15. CORTEX_AI_GUARDRAILS_USAGE_HISTORY        -> USAGE_TIME
        16. CORTEX_DOCUMENT_PROCESSING_USAGE_HISTORY  -> START_TIME
        17. SNOWFLAKE_INTELLIGENCE_USAGE_HISTORY      -> START_TIME
        18. QUERY_ATTRIBUTION_HISTORY                 -> START_TIME

        Snapshot/Configuration tables (no date filtering):
        19. CORTEX_REST_API_RATE_LIMIT_POLICIES       -> (snapshot, full table load)

    NOTE on CORTEX_SEARCH_REFRESH_HISTORY:
        Unlike the other tables, this is NOT exposed as a SNOWFLAKE.ACCOUNT_USAGE view.
    ...
    NOTE on CORTEX_REST_API_RATE_LIMIT_POLICIES:
        This is a snapshot/configuration table (like WAREHOUSE_PARAMETERS, WAREHOUSES,
        COLUMNS, TAGS, etc. in the parent script). It is replicated via full table
        load with no date-based filtering. The replication simply truncates and
        reloads the entire table on each run.
**/
...
CREATE OR REPLACE TRANSIENT TABLE CORTEX_AGENT_USAGE_HISTORY WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.CORTEX_AGENT_USAGE_HISTORY;

CREATE OR REPLACE TRANSIENT TABLE CORTEX_AI_FUNCTIONS_USAGE_HISTORY WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.CORTEX_AI_FUNCTIONS_USAGE_HISTORY;

CREATE OR REPLACE TRANSIENT TABLE CORTEX_SEARCH_BATCH_QUERY_USAGE_HISTORY WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.CORTEX_SEARCH_BATCH_QUERY_USAGE_HISTORY;

CREATE OR REPLACE TRANSIENT TABLE CORTEX_CODE_SNOWSIGHT_USAGE_HISTORY WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.CORTEX_CODE_SNOWSIGHT_USAGE_HISTORY;

CREATE OR REPLACE TRANSIENT TABLE CORTEX_CODE_CLI_USAGE_HISTORY WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.CORTEX_CODE_CLI_USAGE_HISTORY;

CREATE OR REPLACE TRANSIENT TABLE CORTEX_AI_GUARDRAILS_USAGE_HISTORY WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.CORTEX_AI_GUARDRAILS_USAGE_HISTORY;

CREATE OR REPLACE TRANSIENT TABLE CORTEX_DOCUMENT_PROCESSING_USAGE_HISTORY WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.CORTEX_DOCUMENT_PROCESSING_USAGE_HISTORY;

CREATE OR REPLACE TRANSIENT TABLE SNOWFLAKE_INTELLIGENCE_USAGE_HISTORY WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.SNOWFLAKE_INTELLIGENCE_USAGE_HISTORY;

CREATE OR REPLACE TRANSIENT TABLE QUERY_ATTRIBUTION_HISTORY WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.QUERY_ATTRIBUTION_HISTORY;

CREATE OR REPLACE TRANSIENT TABLE CORTEX_REST_API_RATE_LIMIT_POLICIES WITH
DATA_RETENTION_TIME_IN_DAYS=0 LIKE SNOWFLAKE.ACCOUNT_USAGE.CORTEX_REST_API_RATE_LIMIT_POLICIES;
...
replicateData("CORTEX_AGENT_USAGE_HISTORY",                  true, "START_TIME");
replicateData("CORTEX_AI_FUNCTIONS_USAGE_HISTORY",           true, "START_TIME");
replicateData("CORTEX_SEARCH_BATCH_QUERY_USAGE_HISTORY",     true, "START_TIME");
replicateData("CORTEX_CODE_SNOWSIGHT_USAGE_HISTORY",         true, "USAGE_TIME");
replicateData("CORTEX_CODE_CLI_USAGE_HISTORY",               true, "USAGE_TIME");
replicateData("CORTEX_AI_GUARDRAILS_USAGE_HISTORY",          true, "USAGE_TIME");
replicateData("CORTEX_DOCUMENT_PROCESSING_USAGE_HISTORY",    true, "START_TIME");
replicateData("QUERY_ATTRIBUTION_HISTORY",                   true, "START_TIME");

// --- ACCOUNT_USAGE-backed view
replicateData("SNOWFLAKE_INTELLIGENCE_USAGE_HISTORY",        true, "START_TIME");

// --- Snapshot/Configuration table (full table load, no date filtering)
replicateData("CORTEX_REST_API_RATE_LIMIT_POLICIES",         false, "");
...
BEGIN
    ...
    GRANT SELECT ON TABLE CORTEX_AGENT_USAGE_HISTORY                  TO SHARE S_SECURE_SHARE;
    GRANT SELECT ON TABLE CORTEX_AI_FUNCTIONS_USAGE_HISTORY           TO SHARE S_SECURE_SHARE;
    GRANT SELECT ON TABLE CORTEX_SEARCH_BATCH_QUERY_USAGE_HISTORY     TO SHARE S_SECURE_SHARE;
    GRANT SELECT ON TABLE CORTEX_CODE_SNOWSIGHT_USAGE_HISTORY         TO SHARE S_SECURE_SHARE;
    GRANT SELECT ON TABLE CORTEX_CODE_CLI_USAGE_HISTORY               TO SHARE S_SECURE_SHARE;
    GRANT SELECT ON TABLE CORTEX_AI_GUARDRAILS_USAGE_HISTORY          TO SHARE S_SECURE_SHARE;
    GRANT SELECT ON TABLE CORTEX_DOCUMENT_PROCESSING_USAGE_HISTORY    TO SHARE S_SECURE_SHARE;
    GRANT SELECT ON TABLE SNOWFLAKE_INTELLIGENCE_USAGE_HISTORY        TO SHARE S_SECURE_SHARE;
    GRANT SELECT ON TABLE QUERY_ATTRIBUTION_HISTORY                   TO SHARE S_SECURE_SHARE;
    GRANT SELECT ON TABLE CORTEX_REST_API_RATE_LIMIT_POLICIES         TO SHARE S_SECURE_SHARE;
    RETURN 'SUCCESS';
END;
/**
