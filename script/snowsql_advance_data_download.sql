...
--Data isn't copied from FUNCTION_DEFINITION column in FUNCTIONS table and RECORD_DELIMITER column in FILE_FORMATS table. This is done to avoid parsing error due to format.
...
COPY INTO @&{stage_name}/file_formats.csv FROM (SELECT FILE_FORMAT_ID, FILE_FORMAT_NAME, FILE_FORMAT_SCHEMA_ID, FILE_FORMAT_SCHEMA, FILE_FORMAT_CATALOG_ID, FILE_FORMAT_CATALOG, FILE_FORMAT_OWNER, FILE_FORMAT_TYPE, FIELD_DELIMITER, SKIP_HEADER, DATE_FORMAT, TIME_FORMAT, TIMESTAMP_FORMAT, BINARY_FORMAT, ESCAPE, ESCAPE_UNENCLOSED_FIELD, TRIM_SPACE, FIELD_OPTIONALLY_ENCLOSED_BY, NULL_IF, COMPRESSION, ERROR_ON_COLUMN_COUNT_MISMATCH, CREATED, LAST_ALTERED, DELETED, COMMENT, OWNER_ROLE_TYPE FROM SNOWFLAKE.ACCOUNT_USAGE.FILE_FORMATS WHERE CREATED > DATEADD(DAY, -15, CURRENT_TIMESTAMP) ORDER BY CREATED) FILE_FORMAT=(TYPE = CSV FIELD_OPTIONALLY_ENCLOSED_BY = '0x22' ESCAPE_UNENCLOSED_FIELD = NONE) OVERWRITE=TRUE;
COPY INTO @&{stage_name}/functions.csv FROM (SELECT FUNCTION_ID, FUNCTION_NAME, FUNCTION_SCHEMA_ID, FUNCTION_SCHEMA, FUNCTION_CATALOG_ID, FUNCTION_CATALOG, FUNCTION_OWNER, ARGUMENT_SIGNATURE, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH, CHARACTER_OCTET_LENGTH, NUMERIC_PRECISION, NUMERIC_PRECISION_RADIX, NUMERIC_SCALE, FUNCTION_LANGUAGE, VOLATILITY, IS_NULL_CALL, CREATED, LAST_ALTERED, DELETED, COMMENT, IS_EXTERNAL, API_INTEGRATION, CONTEXT_HEADERS, MAX_BATCH_ROWS, COMPRESSION, IMPORTS, HANDLER, TARGET_PATH, RUNTIME_VERSION, PACKAGES, INSTALLED_PACKAGES, OWNER_ROLE_TYPE FROM SNOWFLAKE.ACCOUNT_USAGE.FUNCTIONS WHERE CREATED > DATEADD(DAY, -15, CURRENT_TIMESTAMP) ORDER BY CREATED) FILE_FORMAT=(TYPE = CSV FIELD_OPTIONALLY_ENCLOSED_BY = '0x22' ESCAPE_UNENCLOSED_FIELD = NONE) OVERWRITE=TRUE;
